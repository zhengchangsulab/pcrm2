...
def build_network_dict(network_path):
    network_dict = {}

    nodes_set = set()
    with open(network_path) as fin:
        for line in fin:
...
def main():
    ...
    random_flag = sys.argv[5]


    umotif_cutoff_dict = paser_umotif_score()
    
    network_dict = build_network_dict(network_path)
    binding_score_dict = paser_binding_score(binding_score_name)

    with open("{}.pos_site.keep_pos_site.0.n-1.{}.score".format(crm_name, random_flag), "w") as fout:
