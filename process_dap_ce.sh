...
#dap_path=${cwd_path}"dap_factor/Sorted/extend_1000/Hard_Mask/MOTIF_U_PSM_GT_0.4_SPIC_FOLDER/"
#ce_path=${cwd_path}"ce_factor/Sorted/extend_1000/Hard_Mask/MOTIF_U_PSM_GT_0.4_SPIC_FOLDER/"
...
build_motif_logo_for_each_cluster_script=${cwd_path}"build_motif_logo_for_each_cluster.sh"
...
function convert_graph_format(){
    ...
    spic="ALL_KEEP_MOTIF_PAIR_SPIC_SCORE.GT_0.8"
...
function run_mcl_cluster(){
	...
	#ls ALL_KEEP_MOTIF_PAIR_SPIC_SCORE.GT_0.8.mci.bin
...
<<EOF
for folder in ${dap_path}
do
    dap_ref="/nobackup/zcsu_research/npy/All_In_One_CRM_Project/reference/dap.fa"
    dap_ref_size="/nobackup/zcsu_research/npy/All_In_One_CRM_Project/reference/dap.chrom.sizes"
	...
	#run_sort_merge_bg ${dap_ref} ${dap_ref_size}
...
for folder in ${ce_path}
do
    ce_ref="/nobackup/zcsu_research/npy/All_In_One_CRM_Project/reference/ce.fa"
    ce_ref_size="/nobackup/zcsu_research/npy/All_In_One_CRM_Project/reference/ce.chrom.sizes"
	...
	cd ${folder_name}
	#run_cat_sites_diff_inflat
	#run_sort_merge_bg ${ce_ref} ${ce_ref_size}
	#run_check_unfinished
	cd ../
    done
    cd ${cwd_path}
...
function run_build_database(){
    ...
    #qsub ${pbs}
    bash build_umotif_database_folder_padding_30.sh

}


for folder in ${ce_path} #${dap_path}
do
    cd ${folder}
    for inflat in 1.1 #1.3 1.5 #1.2 1.3 1.4 1.5
    do
	...
	run_convert_to_origin_peak_with_tfbs ce
	pwd
	cd ../
	cd ../../
	cd ../
...
for folder in ${dap_path} #${dap_path}
do
    cd ${folder}
    for inflat in 1.1 #1.3 1.5 #1.2 1.3 1.4 1.5
    do
	...
	cd UMOTIF_BED_2020/
	#run_convert_sites_to_origin_peak
	run_convert_to_origin_peak_with_tfbs dap
